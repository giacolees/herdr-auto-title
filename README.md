...
<p align="center">
  An integration that turns your Claude Code, Codex, Pi, or Command Code conversation into a short title and applies it to the <a href="https://herdr.dev">herdr</a> tab name.
</p>
...
Titles are generated by the same CLI that started the session: `claude -p` for Claude Code, `codex exec` for Codex, `pi -p` for Pi, and `cmd -p` for Command Code.

## Requirements

- Claude Code, Codex, Pi, or Command Code running inside a herdr pane (`HERDR_ENV=1` is set)
- `python3` (standard library only)
- The corresponding `claude`, `codex`, `pi`, or `cmd` CLI on your `PATH`
...
It copies the generator into place. Claude Code and Codex register it on `UserPromptSubmit`; Pi installs an extension that invokes it on user input; Command Code installs a project mod (`transformInput`) that invokes it.
...
It takes effect **from the next session you start**. For a running Pi or Command Code session, run `/reload` after installing.
...
Command Code mods are trust-gated: trust the project when prompted, or run with `cmd --trust`.
