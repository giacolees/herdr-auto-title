...
DEFAULT_MODEL = {CLAUDE: "haiku", CODEX: "gpt-5.4-mini", PI: "", CMD: ""}
...
def user_message_for(conversation_log: str, agent: str) -> str:
    name = {CLAUDE: "Claude Code", CODEX: "Codex", PI: "Pi", CMD: "Command Code"}.get(
        agent, "Claude Code"
    )
    if LANG == JA:
        return (
...
def generate_with_cmd(conversation_log: str, model: str) -> str:
    """cmd -p を最小構成で呼び、タイトル 1 行を得る。"""
    command = [
        "cmd",
        "-p",
        user_message_for(conversation_log, CMD),
        "--no-session",
        "--no-skills",
        "--skip-onboarding",
        "--no-auto-update",
    ...
    if completed is None:
        return ""
    if completed.returncode != 0:
        log(f"cmd -p failed ({completed.returncode}): {completed.stderr[:500]}")
        return ""
    return sanitize_title(completed.stdout)
...
def resolve_backend(agent: str) -> str:
    """どの CLI でタイトルを作るか決める。"""
    if BACKEND in (CLAUDE, CODEX, PI, CMD):
        return BACKEND
    if agent in (PI, CMD):
        return agent
...
def generate_title(conversation_log: str, agent: str, requested_model: str = "") -> str:
    ...
    # Pi/cmd 拡張は現在選択中のモデルを渡せる。別の CLI を明示した場合は渡さない。
    model = MODEL or (requested_model if backend in (PI, CMD) else "") or DEFAULT_MODEL[backend]
    log(f"generating with {backend} ({model or 'default'})")
    if backend == CMD:
        return generate_with_cmd(conversation_log, model)
    if backend == PI:
...
def detect_agent(hook_input: dict) -> str:
    ...
    Pi/cmd 拡張は agent を明示する。Claude Code / Codex の入力はおおむね共通だが、
    turn_id は Codex にしかない。
    """
    if hook_input.get("agent") == CMD:
        return CMD
